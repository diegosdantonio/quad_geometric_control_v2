function animate_3dquad_load(torig, x)
RATE = 10 * 1;
%==========================================
% initialize the animation figure and axes
%==========================================
figure_x_limits = [min(x(:,1))-1 max(x(:,1))+1];
figure_y_limits = [min(x(:,2))-1 max(x(:,2))+1];
figure_z_limits = [min(x(:,3))-1 max(x(:,3))+1] ;
fig1 = figure;
...
figure_x_size = (figure_x_limits(2) - figure_x_limits(1))*2;
figure_y_size = (figure_y_limits(2) - figure_y_limits(1))*2;
...
filename = '.\figures\QuadAnimated.gif';
...
%    plot3_surf(xLd(max(1,i-hist):i, 1), xLd(max(1,i-hist):i, 2), xLd(max(1,i-hist):i, 3), colorRed1, colorRed2, 2, '-');
            ...
            grid on; title({'Geometric Tracking Control of a Quadrotor UA V on SE(3)' , 'Taeyoung Lee, Melvin Leok, and N. Harris McClamroch'});
    ...
    if MAKE_MOVIE, M(:,i) = getframe(fig1); end
